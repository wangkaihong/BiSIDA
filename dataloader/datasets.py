...
class GTAV(data.Dataset):
    ...
    def __init__(self, root, transform=False, im_size=None, mean=True):
        ...
        self.files = []
        fn = os.listdir(osp.join(dataset_dir,'labels'))

        for f in fn:
            f = f.strip()
            img_file = osp.join(dataset_dir, 'images/' + f)
            lbl_file = osp.join(dataset_dir, 'labels/' + f)
            self.files.append({
                'img': img_file,
                'lbl': lbl_file,
            })
    ...
    def __getitem__(self, index):
        ...
        lbl_copy = self.ignore_label * np.ones(lbl.shape, dtype=np.float32)
        for k, v in self.gtav_id_to_trainid.items():
            lbl_copy[lbl == k] = v
...
class CityScapes(data.Dataset):
    ...
    def __init__(self, root, split="train", transform=False, norm=False, scale=(1, 1), val_im_size=(1024, 512), val_lbl_size=(2048, 1024), resize='none', mean=True, transformer=None, validate_type='synthia'):
        self.root = root
        self._transform = transform
        self.split = split
        ...
        if validate_type == 'synthia':

            self.rep_class_names = np.array([
                'road','sidewalk','building','wall','fence','pole','traffic light','traffic sign','vegetation',
                'sky','person','rider','car','bus','motorcycle','bicycle'
            ])

            self.cs_id_to_trainid = {
                7: 0, 8: 1, 11: 2, 12: 3, 13: 4, 17: 5, 19: 6, 20: 7,
                21: 8, 23: 9, 24: 10, 25: 11, 26: 12,
                28: 13, 32: 14, 33: 15
            }
        elif validate_type == 'gtav':

            self.rep_class_names = np.array(['road','sidewalk','building','wall','fence','pole',
                'traffic light','traffic sign','vegetation','terrain','sky','person','rider',
                'car','truck','bus','train','motorcycle','bicycle'
            ])
...
class Synthia_with_CityScapes(data.Dataset):
    ...
    def __init__(self, s_root, c_root, transform=False, im_size=None, mean=True, norm=False, src_transformer=None, tgt_transformer=None, tgt_transformer_2=None, resize_short=1024, k_src=1, k_tgt=1, n_class=16):
        self.s_root = s_root
        self.c_root = c_root
        self._transform = transform
        self.im_size = im_size
    ...
    def __getitem__(self, index):
        ...
        source_full_imgs = [np.array(s, dtype=np.uint8) for s in source_full_imgs] # should nest another layer
        target_full_imgs = [np.array(t, dtype=np.uint8) for t in target_full_imgs] # should nest another layer
        ...
        if self._transform:
            ...
            source_full_imgs = [self.tranform_img(s) for s in source_full_imgs]
            target_full_imgs = [self.tranform_img(t) for t in target_full_imgs]
...
class GTAV_with_CityScapes(data.Dataset):
    ...
    few_class_index = np.array([9, 14, 16])
    ...
    src_class_dist = [
        0.36121355, 0.09311465, 0.19017579, 0.02078988, 0.00716334, 0.01196075,
        0.00155475, 0.00095213, 0.08551179, 0.02424846, 0.15224031, 0.00412264,
        0.00038878, 0.02835164, 0.01275066, 0.00418212, 0.00077335, 0.00039944,
        0.00010596
    ]
    ...
    def __init__(self, g_root, c_root, transform=False, im_size=None, mean=True, norm=False, src_transformer=None, tgt_transformer=None, tgt_transformer_2=None, resize_short=1024, k_src=1, k_tgt=1):
        self.g_root = g_root
        self.c_root = c_root
        self._transform = transform
        self.im_size = im_size
        ...
        s_dataset_dir = osp.join(self.g_root, '')
        self.source_list = []
        self.target_list = []
        fn = os.listdir(osp.join(s_dataset_dir,'labels'))

        for f in fn:
            f = f.strip()
            img_file = osp.join(s_dataset_dir, 'images/' + f)
            lbl_file = osp.join(s_dataset_dir, 'labels/' + f)
            self.source_list.append({
                'img': img_file,
                'lbl': lbl_file,
            })
