...
train_loader = torch.utils.data.DataLoader(
        datasets.Synthia_with_CityScapes(s_root, c_root, transform=True, im_size=train_im_size, src_transformer=src_transformer, tgt_transformer=tgt_transformer, resize_short=resize_short, k_src=k_src, k_tgt=k_tgt),
        batch_size=len(gpus), shuffle=True, num_workers=1, drop_last=True)

val_loader = torch.utils.data.DataLoader(
        datasets.CityScapes(c_root, split="val", transform=True, val_im_size=val_im_size, val_lbl_size=val_lbl_size, resize='resize', transformer=None),
...
balance_function=lambda x: pow((1 / x),1/3.) / 5.
...
out = "logs/train_synthia_vgg_experiment"
...
decoder_pretrained_path = 'saved_models/decoder_0_1.pth.tar'
